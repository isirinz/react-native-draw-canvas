...
const RNDrawCanvas = requireNativeComponent('RNDrawCanvas', DrawCanvas, {
  nativeOnly: {
    nativeID: true,
    onChange: true
  }
});
const DrawCanvasManager = NativeModules.RNDrawCanvasManager || {};
...
class DrawCanvas extends React.Component {
  ...
  clear() {
    this._paths = [];
    this._path = null;
    UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNDrawCanvas.Commands.clear, []);
  }
  ...
  addPath(data) {
    if (this._initialized) {
      ...
      UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNDrawCanvas.Commands.addPath, [
  ...
  deletePath(id) {
    ...
    UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNDrawCanvas.Commands.deletePath, [id]);
  }

  save(imageType, transparent, folder, filename, includeImage, includeText, cropToImageSize) {
    UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNDrawCanvas.Commands.save, [imageType, folder, filename, transparent, includeImage, includeText, cropToImageSize]);
  }
  ...
  getBase64(imageType, transparent, includeImage, includeText, cropToImageSize, callback) {
    if (Platform.OS === 'ios') {
      DrawCanvasManager.transferToBase64(this._handle, imageType, transparent, includeImage, includeText, cropToImageSize, callback)
    } else {
      NativeModules.DrawCanvasModule.transferToBase64(this._handle, imageType, transparent, includeImage, includeText, cropToImageSize, callback)
    }
  }
    ...
    this.panResponder = PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        ...
        UIManager.dispatchViewManagerCommand(
          this._handle,
          UIManager.RNDrawCanvas.Commands.newPath,
          [
            this._path.id,
        ...
        UIManager.dispatchViewManagerCommand(
          this._handle,
          UIManager.RNDrawCanvas.Commands.addPoint,
          [
      ...
      onPanResponderMove: (evt, gestureState) => {
        if (!this.props.touchEnabled) return
        if (this._path) {
          UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNDrawCanvas.Commands.addPoint, [
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        UIManager.dispatchViewManagerCommand(this._handle, UIManager.RNDrawCanvas.Commands.endPath, [])
      },
...
DrawCanvas.MAIN_BUNDLE = Platform.OS === 'ios' ? UIManager.RNDrawCanvas.Constants.MainBundlePath : '';
DrawCanvas.DOCUMENT = Platform.OS === 'ios' ? UIManager.RNDrawCanvas.Constants.NSDocumentDirectory : '';
DrawCanvas.LIBRARY = Platform.OS === 'ios' ? UIManager.RNDrawCanvas.Constants.NSLibraryDirectory : '';
DrawCanvas.CACHES = Platform.OS === 'ios' ? UIManager.RNDrawCanvas.Constants.NSCachesDirectory : '';
